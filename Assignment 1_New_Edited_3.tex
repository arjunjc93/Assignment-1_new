...
\end{center}
\\\\
The vector equation of a line passing through two points with position vector \vec{A} \& \vec{B} is:
\[\vec{r}=\vec{A}+t(\vec{B}-\vec{A})\]
where, t is a scalar.
\\
Let $\vec{T}$ represent the line \( 3x+4y-6=0\),\\\\
\therefore
\[\vec{T}=\vec{P}+t(\vec{Q}-\vec{P})\]
\[\Rightarrow\vec{T}=\bigg[\bigg[\myvec{2\\3}+t\bigg[\bigg[\myvec{-2\\0}\bigg]-\bigg[\myvec{2\\3}\bigg]\bigg]\bigg]\]\\
\therefore
\[\vec{T}=\bigg[\bigg[\myvec{2\\3} +t\bigg[\myvec{4\\3}\bigg]\bigg]\bigg]\]
\\
\\
where, \[x=2+4t\]\[y=3+3t\]
\\
Let $\vec{U}$ represent the line \( 3x-y+9=0\),\\\\
\therefore
...
where, \[x=2+t\]\[y=3-3t\]
\\
Let $\vec{V}$ represent the x axis \[y=0\],\\\\
\therefore
